...
\begin{note}
One can also ``derive'' (informally) the quadratic variation of It\^o integral
through ``differential rules'':
\[
\odif{I_t}\odif{I_t}
=(\Delta_t\odif{W_t})(\Delta_t\odif{W_t})
=\Delta_t^{2}\odif{W_t}\odif{W_t}
\overset{\text{(differential rule)}}{=}\Delta_t^{2}\odif{t}.
\]
Quite often, manipulating the differentials based on ``differential rules''
would indeed yield correct results. While it may not be completely formal, it
serves as a very practically useful method for ``quick derivations'' or
``making guesses''.
\end{note}
...
\end{theorem}
\begin{pf}
Consider any partition \(\Pi=\{t_0,t_1,\dotsc,t_n\}\) of \([0,T]\), with
\(0=t_0\le t_1\le\dotsb\le t_n=T\). The main idea in the proof is to express
the difference \(f(T,W_T)-f(0,W_0)\) in terms of \emph{Taylor series} of
approximated differences within subintervals that involve partial derivatives.
...
\emph{Differential form of It\^o formula.} While the proof of the It\^o formula
above is mathematically ``correct'', it is rather lengthy and not so
insightful.  A much more intuitive and insightful (but a bit informal)
``derivation'' of the It\^o formula is based on the idea of \emph{differential
form} (again). The differential form of the It\^o formula is
\[
\boxed{\odif{{f(t,W_t)}}=f_{t}(t,W_t)\odif{t}+f_{x}(t,W_t)\odif{W_t}
...
+\frac{1}{2}f_{xx}(t,W_t)\odif{t}.
\end{align*}
Although this kind of ``derivation'' is informal, it often serves as an useful
way for deriving (and memorizing) formulas about It\^o integrals efficiently.
...
In differential form, we write \(\odif{X_t}=\Delta_t\odif{W_t}+\Theta_t\odif{t}\).
...
\end{proposition}
\begin{pf}
Let \(I_t=\int_{0}^{t}\Delta_u\odif{W_u}\) and \(R_t=\int_{0}^{t}\Theta_u\odif{u}\),
and \(\Pi=\{t_0,t_1,\dotsc,t_n\}\) be a partition of \([0,t]\) with \(0=t_0\le
t_1\le\dotsb\le t_n=t\). By \labelcref{it:ito-int-gen-prop} and the fundamental
theorem of calculus, both \(I_t\) and \(R_t\) are continuous in \(t\). Next, write
\[
...
\emph{Differential form.} \Cref{prp:ito-process-quad-var} can again be
expressed in the differential form: \(\odif{X_t}\odif{X_t}=\Delta^{2}_t\odif{t}\).
Here we can also have a quick ``derivation'' of this result by manipulating the
differentials:
\[
...
a \(\{\mathcal{F}_t\}\)-adapted process. Then, we define the \defn{integral
with respect to an It\^o process} by
\[
...
\begin{pf}
Consider any partition \(\Pi=\{t_0,t_1,\dotsc,t_n\}\) of \([0,T]\), with
\(0=t_0\le t_1\le\dotsb\le t_n=T\). Similar to the proof of
\Cref{thm:ito-fmla-bm}, we first apply Taylor's theorem to get
\begin{align*}
f(T,X_T)-f(0,X_0)&=\sum_{j=0}^{n-1}\left(f(t_{j+1},X_{t_{j+1}})-f(t_{j},X_{t_j})\right) \\
\overset{\text{(Taylor)}}&{=}\sum_{j=0}^{n-1}f_{t}(t_j,X_{t_j})(t_{j+1}-t_{j})
+\sum_{j=0}^{n-1}f_{x}(t_j,X_{t_j})(X_{t_{j+1}}-X_{t_{j}}) \\
&\quad+\frac{1}{2}\sum_{j=0}^{n-1}f_{xx}(t_j,X_{t_j})(X_{t_{j+1}}-X_{t_j})^{2}
+\sum_{j=0}^{n-1}f_{tx}(t_j,X_{t_j})(t_{j+1}-t_{j})(X_{t_{j+1}}-X_{t_j}) \\
&\quad+\frac{1}{2}\sum_{j=0}^{n-1}f_{tt}(t_j,X_{t_j})(t_{j+1}-t_{j})^{2}
+\text{higher order remainder term}.
\end{align*}
Like before, consider the limit as \(\|\Pi\|\to 0\) and consider the six terms
on the right-hand side one by one.  Let \(I_t=\int_{0}^{t}\Delta_u\odif{W_u}\)
and \(R_t=\int_{0}^{t}\Theta_u\odif{u}\).
\begin{itemize}
\item \(\lim_{\|\Pi\|\to
0}\sum_{j=0}^{n-1}f_{t}(t_j,X_{t_j})(t_{j+1}-t_{j})=\int_{0}^{T}f_{t}(t,X_t)\odif{t}\) (by definition).
\item We have
\begin{align*}
&\quad\lim_{\|\Pi\|\to 0}\sum_{j=0}^{n-1}f_{x}(t_j,X_{t_j})(X_{t_{j+1}}-X_{t_{j}}) \\
&=\lim_{\|\Pi\|\to 0}\sum_{j=0}^{n-1}f_{x}(t_j,X_{t_j})(I_{t_{j+1}}-I_{t_j}+R_{t_{j+1}}-R_{t_j}) \\
&=\lim_{\|\Pi\|\to 0}\sum_{j=0}^{n-1}f_{x}(t_j,X_{t_j})(I_{t_{j+1}}-I_{t_j})
+\lim_{\|\Pi\|\to 0}\sum_{j=0}^{n-1}f_{x}(t_j,X_{t_j})(R_{t_{j+1}}-R_{t_j}) \\
\overset{\text{(limiting argument)}}&{=}
\lim_{\|\Pi\|\to 0}\sum_{j=0}^{n-1}f_{x}(t_j,X_{t_j})(\Delta_{t_j}\cdot (W_{t_{j+1}}-W_{t_j}))
+\lim_{\|\Pi\|\to 0}\sum_{j=0}^{n-1}f_{x}(t_j,X_{t_j})(\Theta_{t_j}\cdot (t_{j+1}-t_j)) \\
&=\int_{0}^{T}f_{x}(t,X_t)\Delta_t\odif{W_t}+\int_{0}^{T}f_{x}(t,X_t)\Theta_t\odif{t}
\overset{\text{(definition)}}{=}\int_{0}^{T}f_{x}(t,X_t)\odif{X_t}.
\end{align*}
\item We have
\begin{align*}
\quad\lim_{\|\Pi\|\to 0}\frac{1}{2}\sum_{j=0}^{n-1}f_{xx}(t_j,X_{t_j})(X_{t_{j+1}}-X_{t_j})^{2}
\underset{\text{(limiting argument)}}{\overset{\text{(\Cref{prp:ito-process-quad-var})}}{=}}
\frac{1}{2}\int_{0}^{T}f_{xx}(t,W_t)\Delta_{t}^{2}\odif{t}.
\end{align*}
\item Similar to \Cref{thm:ito-fmla-bm}, the rest of the terms all converge to
\(0\) as \(\|\Pi\|\to 0\).
\end{itemize}
\end{pf}

\emph{Differential form.} Again, more insights can be gained by expressing the
It\^o formula in differential form:
\[
\boxed{\odif{{f(t,X_t)}}=f_{t}(t,X_t)\odif{t}+f_{x}(t,X_t)\odif{X_t}
...
\item\label{it:zero-drift-mart} For the geometric Brownian motion, in case the
...
is also a martingale. This gives a rise to a very useful technique for showing
...
Let \(\{W_t\}\) be a Brownian motion and \(\Delta_{u}\) be a nonrandom function
of \(u\). Then, we have
\[
...
\begin{enumerate}
\item In STAT3910, we have studied option pricing under the Black-Scholes
...
under the Black-Scholes model. Its derivation is based on a replication argument.
...
\]
where \(\alpha\in\R\) and \(\sigma>0\) are constants. Also, 
\item \textbf{Constructing a self-financing portfolio.} In the derivation of
Black-Scholes equation, a \emph{self-financing} portfolio is utilized for
replicating the option payoffs. So, we will first develop such portfolio here
as a preparation. At each time \(t\), let \(X_t\) denote the investor's
portfolio value and \(\Delta_t\) denote the number of shares of stock held by
the investor. Here, we suppose that \(\{\Delta_t\}\) is adapted to a filtration
\(\{\mathcal{F}_t\}\) for the Brownian motion \(\{W_t\}\), and at each time
\(t\), the non-stock component of the portfolio, with value \(X_t-\Delta_t
S_t\), is all invested in the bond.

Due to  the self-financing property of the portfolio, the ``change''
\(\odif{X_t}\) for the investor's portfolio at time \(t\) would source from the
following two factors: (i) the gain \(\Delta_t\odif{S_t}\) on the stock
position and (ii) the interest earning \(r(X_t-\Delta_tS_t)\odif{t}\) on the
bond position.  Hence, we can write
\begin{align*}
...
function \(c(t,S_t)\) is of class \(C^2\). Then, the function \(c(t,S_t)\)
satisfies the following \emph{Black-Scholes partial differential equation
(PDE)}:
\[
c_{t}(t,S_t)+rS_t c_{x}(t,S_t)+\frac{1}{2}\sigma^{2}S_t^{2}c_{xx}(t,S_t)
=rc(t,S_t)\quad\text{for
all \(t\in [0,T)\),}
\]
with the terminal condition \(c(T,S_T)=(S_T-K)_{+}\).
\end{theorem}
\begin{pf}
...
\end{align*}
Now, we start the replication argument. The key is to construct a replicating
portfolio with the initial capital \(X_0\), that invests in the stock and bond
such that \(X_t=c(t,S_t)\) for every time \(t\in [0,T]\) \emph{(replicating the
call)}. This requirement can be expressed in differential form as, for every
\(t\in [0,T)\), \(\odif{(e^{-rt}X_t)}=\odif{{(e^{-rt}c(t,S_t))}}\) (meaning
that \(e^{-rt}X_t-X_0=e^{-rt}c(t,S_t)-c(0,S_0)\)), with \(X_0=c(0,S_0)\); this
would then imply \(X_t=c(t,S_t)~\forall t\in [0,T)\), and taking the limit
\(t\to T^{-}\) would also yield \(X_T=c(T,S_T)\) by the continuity of
\(X_t\)\footnote{\(X_t\) is continuous in \(t\) since both integrals in the
expression of It\^o process are continuous in the upper limit \(t\), by
\labelcref{it:ito-int-gen-prop} and the fundamental theorem of calculus.} and
\(c(t,x)\).

By the SDE above (for discounted call price) and the SDE for discounted
portfolio price, we can then write
\begin{align*}
...
+e^{-rt}\sigma S_{t}c_{x}(t,S_t)\odif{W_t}.
\end{align*}
To make such SDE hold, one way to equate the drift and volatility terms
from both sides. Equating the volatility terms gives
\[
\Delta_t=c_{x}(t,S_t)\quad\text{for all \(t\in [0,T)\),}
\]
which is known as the \defn{delta-hedging rule} (recall that the partial
derivative \(c_{x}(t,S_t)\) is known as the \defn{delta} of the call). Next,
equating the drift terms with \(\Delta_t=c_{x}(t,S_t)~\forall t\in [0,T)\)
yields, for all \(t\in [0,T)\),
\begin{align*}
...
\end{align*}
which gives us the Black-Scholes partial differential equation. Furthermore,
we know that \(c(T,x)\) is simply the time-\(T\) payoff of the call, which is
\((S_T-K)_{+}\).
\end{pf}
...
and is indeed the \emph{Black-Scholes formula} studied in STAT3910, namely
\[
...
independent.\footnote{TODO: what definition?
Here, this independence means that for all \(t\), the random variables \(W_{t,i}\)
and \(W_{t,j}\) are independent.
%This independence means that for all \(n\in\N\) and
%\(t_1,\dotsc,t_n\), the random vectors \(\vect{W}_{,i}=(W_{t_1,i},\dotsc,W_{t_n,i})\)
%and \(\vect{W}_{,j}=(W_{t_1,j},\dotsc,W_{t_n,j})\) are independent, which in turn means
%that the \(\sigma\)-algebras generated by them, \(\vect{W}_{,i}^{-1}(\mathcal{B}(\R^{n}))\)
%and \(\vect{W}_{,j}^{-1}(\mathcal{B}(\R^{n}))\), are independent.
}
...
\(\{W_{t,j}\}\), given by \[[W_{i},W_{j}]_{T}= \lim_{\|\Pi\|\to
...
\Cref{prp:d-dim-bm-zero-cross-var} as \(\odif{W_{t,i}}\odif{W_{t,j}}\), which
...
\]
where \(\Theta_{u}\), \(\sigma_{u,1}\), and \(\sigma_{u,2}\) are adapted to
\(\{\mathcal{F}_t\}\). In differential form, we can write
\[
\odif{X_t}=\Theta_{t}\odif{t}+\sigma_{u,1}\odif{W_{t,1}}+\sigma_{u,2}\odif{W_{t,2}}.
\]
\begin{note}
...
\begin{align*}
\odif{X_t}&=\Theta_{t,1}\odif{t}+\sigma_{u,1,1}\odif{W_{t,1}}+\sigma_{u,1,2}\odif{W_{t,2}}, \\
\odif{Y_t}&=\Theta_{t,2}\odif{t}+\sigma_{u,2,1}\odif{W_{t,1}}+\sigma_{u,2,2}\odif{W_{t,2}}.
\end{align*}
...
\begin{align*}
\odif{X_t}\odif{X_t}&=(\sigma_{t,1,1}^{2}+\sigma_{t,1,2}^{2})\odif{t}, \\
\odif{Y_t}\odif{Y_t}&=(\sigma_{t,2,1}^{2}+\sigma_{t,2,2}^{2})\odif{t}, \\
\odif{X_t}\odif{Y_t}&=(\sigma_{t,1,1}\sigma_{t,2,1}+\sigma_{t,1,2}\sigma_{t,2,2})\odif{t}.
\end{align*}
These can be more precisely expressed as:
\begin{align*}
[X,X]_{T}&=\int_{0}^{T}(\sigma_{t,1,1}^{2}+\sigma_{t,1,2}^{2})\odif{t}, \\
[Y,Y]_{T}&=\int_{0}^{T}(\sigma_{t,2,1}^{2}+\sigma_{t,2,2}^{2})\odif{t}, \\
[X,Y]_{T}&=\int_{0}^{T}(\sigma_{t,1,1}\sigma_{t,2,1}+\sigma_{t,1,2}\sigma_{t,2,2})\odif{t}.
\end{align*}
...
``\(\int_{0}^{T}f_{xy}(t,X_t,Y_t)(\sigma_{t,2,1}^{2}+\sigma_{t,2,2}^{2})\odif{t}\)''.
\end{theorem}
\begin{pf}
The proof is similar to the one for \Cref{thm:ito-fmla-ito-process}, but is
considerably more tedious. So we will omit it here.
\end{pf}

\emph{Differential form.} The integral form of this It\^o formula is rather
complicated. Usually, for It\^o formula in this level of
generality, it would be much more convenient to work with the differential
form, given by
\begin{align*}
\odif{{f(t,X_t,Y_t)}}&=
...
\end{theorem}
\begin{pf}
The ``\(\Rightarrow\)'' direction follows by the definition of Brownian motion
and \Cref{prp:bm-unit-rate-quad-var}. So it remains to prove the
``\(\Leftarrow\)'' direction. By the definition of Brownian motion, it suffices
to prove that \(M_t\sim\ndist{0,t}\) for all \(t\ge 0\) with the three
conditions assumed. 
...
analogous fashion as the one for Brownian motion. Particularly, it can be shown
...
\(\expv{e^{uM_t-u^{2}t/2}}=\expv{f(t,M_t)}=\expv{\expv{f(t,M_t)}|\mathcal{F_0}}
...
\([M_{1},M_{1}]_{t}=[M_{2},M_{2}]_{t}=t\) and
\([M_1,M_2]_{t}=0\) for all \(t\ge 0\).
\end{enumerate}
\end{theorem}
\begin{pf}
...
moment generating function of \(M_{t,1}\) and \(M_{t,2}\) can be written as a
product of the two respective marginal moment generating functions.  Hence,
\(M_{t,1}\) and \(M_{t,2}\) are independent. Hence, the Brownian motions
\(\{M_{t,1}\}\) and \(\{M_{t,2}\}\) are independent.
\end{pf}
