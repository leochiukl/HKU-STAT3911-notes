...
martingale). Let \(Y\) be a \(\mathcal{F}_t\)-measurable random
variable.
\begin{enumerate}
\item For all \(t\in [0,T]\), \(\texpv{Y}=\expv{YZ_t}\).
\item For all \(0\le s\le t\le T\), \(\texpv{Y|\mathcal{F}_s}=\expv{YZ_t|\mathcal{F}_s}/Z_s\).
\end{enumerate}
\end{lemma}
\begin{pf}
\begin{enumerate}
...
\item\label{it:rn-meas} \textbf{Risk-neutral measure.} The probability measure
\(\tpr\) from the Girsanov theorem is indeed a \emph{risk-neutral measure}.
Like \labelcref{it:rn-meas-absorb-drift}, it can be interpreted as being
designed for absorbing the drift term. But here, rather than working with only
the geometric Brownian motion, we are considering a more general setting with
\emph{generalized} geometric Brownian motion.
...
for deriving them. Let \(c(t,S_t)\) denote the time-\(t\) value of a \(T\)-year
\(K\)-strike European call. Assume that \(r_t=r\) and \(\sigma_t=\sigma\) for
all \(t\in [0,T]\), where \(r\) and \(\sigma>0\) are constants.
\begin{enumerate}
\item \emph{(Deriving Black-Scholes formula)} Previously the Black-Scholes
formula is derived as the solution to the Black-Scholes equation. A more direct
way of deriving the formula is to utilize the \emph{risk-neutral pricing
formula} from \labelcref{it:rn-pricing}; such approach is indeed used in
STAT3905/STAT3910, where the Black-Scholes formula is derived through suitable
algebraic manipulations. The key idea is as follows.
\[
c(t,S_t)\overset{\text{\labelcref{it:rn-pricing}}}{=}\expv{e^{-r(T-t)}(S_T-K)_{+}|\mathcal{F}_t}
\overset{\text{(algebra)}}{=}
...
applying the converse of \labelcref{it:zero-drift-mart} (which can be shown to
hold), i.e., being a martingale implies that the drift term is zero.

Consider the process \(\{c(t,S_t)\}\) of call prices, and its corresponding
replicating portfolio \(\{X_t\}\), satisfying that \(X_t=c(t,S_t)\) for all
\(t\in[0,T]\). From the proof of \Cref{thm:bs-eqn}, we know
\begin{align*}
\odif{{c(t,S_t)}}
&=\left[c_{t}(t,S_t)+\alpha S_tc_{x}(t,S_t)
...
\left[c_{t}(t,S_t)+\alpha S_tc_{x}(t,S_t)
+\frac{1}{2}\sigma^{2}S_{t}^{2}c_{xx}(t,S_t)\right]\odif{t}
+\sigma S_{t}c_{x}(t,S_t)\left(\odif{\widetilde{W}_t}-\frac{\alpha-r}{\sigma}\odif{t}\right) \\
&=\left[c_{t}(t,S_t)+\vc{r}S_tc_{x}(t,S_t)
+\frac{1}{2}\sigma^{2}S_{t}^{2}c_{xx}(t,S_t)\right]\odif{t}
+\sigma S_{t}c_{x}(t,S_t)\odif{\widetilde{W}_t}.
\end{align*}
Hence, letting \(D_t=e^{-rt}\) denote the time-\(t\) discount factor and
applying It\^o product rule, we have
\begin{align*}
\odif{{(D_tc(t,S_t))}}
&=c(t,S_t)\underbrace{\odif{D_t}}_{\mathclap{-rD_t\odif{t}}}
+D_t\odif{{c(t,S_t)}}+\underbrace{\odif{D_t}\odif{{c(t,S_t)}}}_{0} \\
&=-rD_tc(t,S_t)\odif{t}+D_t\left\{
\left[c_{t}(t,S_t)+rS_tc_{x}(t,S_t)
+\frac{1}{2}\sigma^{2}S_{t}^{2}c_{xx}(t,S_t)\right]\odif{t}
+\sigma S_{t}c_{x}(t,S_t)\odif{\widetilde{W}_t}\right\} \\
&=D_t\left[c_{t}(t,S_t)+rS_tc_{x}(t,S_t)
+\frac{1}{2}\sigma^{2}S_t^{2}c_{xx}(t,S_t)-rc(t,S_t)\right]\odif{t}
+\sigma D_tS_{t}c_{x}(t,S_t)\odif{\widetilde{W}_t}.
\end{align*}
By \labelcref{it:self-fin-port-rn}, we know that \(\{D_tX_t\}=\{D_tc(t,S_t)\}\)
is a martingale. Therefore, applying the converse of
\labelcref{it:zero-drift-mart} gives
\[
c_{t}(t,S_t)+rS_tc_{x}(t,S_t)
+\frac{1}{2}\sigma^{2}S_t^{2}c_{xx}(t,S_t)-rc(t,S_t)=0.
\]
...
\end{enumerate}
To analyze the second condition that \(\{D_tS_t^{(i)}\}\) is a
...
\]
where each \(\{\Theta_{t}^{(j)}\}\) is a process such that
\(\alpha_{t}^{(i)}-r_t =\sum_{j=1}^{d}\sigma_{t}^{(ij)}\Theta_{t}^{(j)}\) for
all \(i=1,\dotsc,m\), known as the \defn{market price of risk equations}.

It can be shown that such process \(\{\Theta_{t}^{(j)}\}\) exists for all
\(j=1,\dotsc,d\) (i.e., a solution to the market price of risk equations
exists) iff there is no arbitrage in the market. \begin{note} The
``\(\Rightarrow\)'' direction can be established by the first fundamental
