...
would only have access to \emph{information} available as of time \(t\); we
...
would not ``accidentally'' use some information that cannot be accessed.
...
(``incorporating information on hand''), which are fundamental for our later
...
\defn{sub-\(\sigma\)-algebra} of \(\mathcal{F}\)) for all \(t\in[0,T]\), where
\(T>0\) is a constant.  If we have \(\mathcal{F}_s\subseteq \mathcal{F}_t\) for
all \(0\le s\le t\le T\), then the collection \(\{\mathcal{F}_t\}\) (or
\(\{\mathcal{F}_{t}\}_{t\in[0,T]}\) to be more specific) is said to be a
\defn{filtration}.  In such case, the quadruple
\((\Omega,\mathcal{F},\{\mathcal{F}_t\},\pr)\) is called a \defn{filtered
probability space}.

At time \(t\in[0,T]\), we would know for each set in \(\mathcal{F}_t\) whether
...
\(\{\mathcal{F}_t\}_{t\in [0,T]}\) would be treated as a model of the accrual
...
[0,T]\). This gives rise to the definition of \emph{adapted stochastic process}.

Let \((\Omega,\mathcal{F},\{\mathcal{F}_t\}_{t\in[0,T]},\pr)\) be a filtered probability
space. Then a stochastic process \(\{X_t\}_{t\in[0,T]}\) is said to be
\defn{adapted (to the filtration \(\{\mathcal{F}_t\}_{t\in[0,T]}\))} if \(X_t\) is
\(\mathcal{F}_t\)-measurable for all time \(t\in[0,T]\).
\end{enumerate}
...
impacts on the \emph{probabilities} about \(X\).
...
are called \defn{independent} if \(\prob{A_1\cap\dotsb A_n} =\prob{A_1}\dotsb\prob{A_n}\) for all
\(A_1\in\mathcal{G}_1,\dotsc, A_n\in\mathcal{G}_n\). Also, the sequence of
\(\sigma\)-algebras \(\mathcal{G}_1,\mathcal{G}_2,\dotsc\) is called \defn{independent}
if the \(n\) \(\sigma\)-algebras \(\mathcal{G}_1,\dotsc,\mathcal{G}_n\) are
independent for every \(n\in\N\).

Let \(X_1,X_2,\dotsc\) be random variables. The random variables
\(X_1,\dotsc,X_n\) are said to be independent if \(\prob{X_1\in B_1\cap\dotsb
...
\(Y\) are independent random variables, then \(\expv{XY}=\expv{X}\expv{Y}\)
provided that \(\expv{|XY|}<\infty\).
...
Multiplying the probabilities of each of the events
\(A_{HH},A_{HT},A_{TH},A_{TT}\) in the corresponding cases from the formula
above gives
\begin{align*}
...
variable that is nonnegative or integrable. The \defn{conditional expectation
...
variables below.
\begin{enumerate}
\item \emph{(existence and uniqueness)} Given any nonnegative or integrable
random variable \(X\), there always exists a conditional expectation
\(\expv{X|\mathcal{G}}\) (i.e., a random variable satisfying both the
\emph{measurability} and \emph{partial averaging} requirements). Moreover, such
random variable is unique in \emph{almost sure} sense, i.e., if \(Y\) and \(Z\)
both qualify to be the conditional expectation of \(X\) given \(\mathcal{G}\),
then \(Y\eqas Z\).
...
\item \emph{(monotonicity)} If \(X\le Y\) a.s., then
\(\expv{X|\mathcal{G}}\le\expv{Y|\mathcal{G}}\) a.s.
\item \emph{(taking out what is known (TOWIK))} If \(X\), \(Y\), and \(XY\) are
...
real-valued function on \(\R\) and \(X\) is integrable, then
\(\varphi(\expv{X|\mathcal{G}})\le\expv{(\varphi(X)|\mathcal{G}}\).
\item \label{it:indp-lma} \emph{(independence lemma)} Suppose \(X_1,\dotsc,X_k\) are
\(\mathcal{G}\)-measurable and each of \(Y_1,\dotsc,Y_{\ell}\) is independent
of \(\mathcal{G}\). Let \(f(x_1,\dotsc,x_k,y_1,\dotsc,y_{\ell})\) be a
measurable function and define
\(g(x_1,\dotsc,x_k):=\expv{f(x_1,\dotsc,x_k,Y_1,\dotsc,Y_{\ell})}\). Then,
\(\expv{f(X_1,\dotsc,X_k,Y_1,\dotsc,Y_{\ell})|\mathcal{G}}=g(X_1,\dotsc,X_k)\).
\begin{intuition}
...
\begin{pf}
\begin{enumerate}
\item We shall omit the proof for the existence part and prove only the
uniqueness part. Assume \(Y\) and \(Z\) both satisfy the measurability and
partial averaging requirements. With \(Y\) and \(Z\) both being
\(\mathcal{G}\)-measurable, it can be shown that \(Y-Z\) is
\(\mathcal{G}\)-measurable also.
\begin{intuition}
If the information contained in \(\mathcal{G}\) is sufficient for determining
the values of \(Y(\omega)\) and \(Z(\omega)\), then it should also be
sufficient for determining the value of \(Y(\omega)-Z(\omega)\).
\end{intuition}

Now, consider the set
\(A=\{\omega\in\Omega:Y(\omega)-Z(\omega)>0\}\in\mathcal{G}\) (it is in
\(\mathcal{G}\) since \(Y-Z\) is \(\mathcal{G}\)-measurable). By the partial
averaging property, we have
\[
\int_{A}^{}Y(\omega)\odif{\prob{\omega}}=\int_{A}^{}X(\omega)\odif{\prob{\omega}}
=\int_{A}^{}Z(\omega)\odif{\prob{\omega}},
\]
which implies \(\int_{A}^{}(Y(\omega)-Z(\omega))\odif{\prob{\omega}}=0\). By
construction of \(A\), the integrand is always positive, forcing that
\(\prob{A}=\prob{Y>Z}=0\). Using the same argument with the roles of \(Y\) and
\(Z\) reversed, we can similarly get \(\prob{Y<Z}=0\). Hence, we must have
\(\prob{Y=Z}=1\), i.e., \(Y\eqas Z\).
...
\item Consider the set \(A=\Omega\in\mathcal{G}\). By the partial averaging
property, we have
\[
\expv{\expv{X|\mathcal{G}}}
=\int_{\Omega}^{}\expv{X|\mathcal{G}}\odif{\pr}
\overset{\text{(partial averaging)}}{=}\int_{\Omega}^{}X\odif{\pr}
=\expv{X}.
\]
\item Firstly, \(\expv{X}\) is \(\mathcal{G}\)-measurable, as a deterministic constant.
...
\end{enumerate}
\end{pf}
\item \textbf{Martingales and Markov processes.} We close \Cref{sect:info-cond}
by introducing some terminologies that relate \emph{conditioning} and
\emph{stochastic processes}, which will be revisited in later sections.

Let \((\Omega,\mathcal{F},\{\mathcal{F}_t\}_{t\in[0,T]},\pr)\) be a filtered
probability space, and let \(\{X_t\}_{t\in[0,T]}\) be a stochastic
process adapted to \(\{\mathcal{F}_t\}\).

\begin{itemize}
\item \emph{Martingale/submartingale/supermartingale:} The process \(\{X_t\}\) is
said to be:
\begin{itemize}
\item a \defn{martingale} if \(X_s=\expv{X_t|\mathcal{F}_s}\) for all \(0\le s\le t\le T\).
\item a \defn{submartingale} if \(X_s\le\expv{X_t|\mathcal{F}_s}\) for all \(0\le s\le t\le T\).
\item a \defn{supermartingale} if \(X_s\ge\expv{X_t|\mathcal{F}_s}\) for all \(0\le s\le t\le T\).
\end{itemize}
\begin{intuition}
A \emph{martingale} has \underline{no} tendency to rise \faIcon{arrow-up} or fall
\faIcon{arrow-down}; a \emph{submartingale} has \underline{no} tendency to fall \faIcon{arrow-down};
a \emph{supermartingale} has \underline{no} tendency to rise \faIcon{arrow-up}.
\end{intuition}
\item \emph{Markov process:} The process \(\{X_t\}\) is said to be a
\defn{Markov process} if for all \(0\le s\le t\le T\) and all nonnegative
measurable functions \(f\), there exists a measurable function \(g\) such that
\(\expv{f(X_t)|\mathcal{F}_s}=g(X_s)\).
\begin{intuition}
...
showing that a stochastic process is a Markov process; see \Cref{sect:brownian-motions}.
\end{itemize}
\end{enumerate}
