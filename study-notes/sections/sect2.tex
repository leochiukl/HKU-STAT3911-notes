...
both qualify to be a conditional expectation of \(X\) given \(\mathcal{G}\),
then \(Y\eqas Z\).
...
all nonnegative measurable functions \(f\), there exists a measurable function
\(g\) such that \(\expv{f(X_t)|\mathcal{F}_s}=g(X_s)\)'' \emph{(\defn{Markov
property})}, then \(\{X_t\}\) is called a \defn{(\(\{\mathcal{F}_t\}\)-)Markov
process}.
