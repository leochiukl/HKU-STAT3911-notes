...
\(\omega\in\Omega\) (call it \(W\)). Then, we have \([W,W]_T\eqas T\) for all \(T>
0\).\footnote{To have the almost sure equality, the limit is
supposed to be taken with respect to a sequence of partitions whose norms
converge to zero ``sufficiently fast''.}
\end{proposition}
\begin{pf}
...
\end{pf}
\item \textbf{Differential rules for Brownian motions.} While the function
\(t\mapsto W_t(\omega)\) with \(\omega\in\Omega\) fixed is generally not
differentiable, there are still some \emph{informal} \warn{} ``differential
rules'' available for the Brownian motion, which will be helpful for handling
\emph{stochastic calculus} (see \Cref{sect:stochatic-calc}):
\begin{enumerate}[label={(\arabic*)}]
...
\end{enumerate}
\end{pf}
\item \textbf{First passage time and stopping time.} Let \(m\) be a real number
and \(\{W_t\}\) be a Brownian motion. Then the \defn{first passage time of
Brownian motion to level \(m\)} is \(\tau_m:=\inf\{t\ge 0: W_t=m\}\).
Particularly, if \(W_t\ne m\) for all \(t\ge 0\), then the first passage time
would be \(\tau_m=\inf\varnothing=\infty\).
...
Next, for the case where \(m<0\), note that \(\tau_{m}\) and
\(\tau_{|m|}=\tau_{-m}\) have the same distribution by symmetry of the Brownian
...
\begin{enumerate}
\item \textbf{Idea of reflection principle.} To derive further distributions
...
probability to have a path realized from any one of these types. This leads
to the \emph{reflection equality}:
...
\begin{pf}
\begin{enumerate}
\item Fix any \(m>0\) and \(w\le m\). Note that \(M_t\ge m\) iff \(\tau_m\le
t\). Hence, we have
\[
