...
calculus}, which is touched upon in STAT3911. As suggested by its name, it is a
new type of calculus ``with randomness''; for instance, rather than
differentiating with respect to a deterministic time variable, we differentiate
with respect to a \emph{random variable} (you will learn how to make sense of
this in STAT3911).
...
\(\mathcal{A}\), and (ii) for any \(\sigma\)-algebra \(\mathcal{F}'\) on
...
function satisfying:
\begin{enumerate}[label={(\arabic*)}]
\item \(\pr\) is a function from \(\mathcal{F}\) to \([0,1]\).
...
\(\R\).
\begin{note}
Sometimes, in the definition of random variable, we may be allowed to
replace \(\R\) by \(\eR=[-\infty,\infty]\), the set of extended real numbers.
In respond to this change, we would change \(\mathcal{B}\) to the Borel
...
\defn{\(\mathcal{F}\)-measurable}. So we can say that a random variable is a
\(\mathcal{F}\)-measurable function. For a generic function \(f:\R\to\R\), we
can define measurability in a similar fashion: It is \defn{(Borel-)measurable}
if \(f^{-1}(\mathcal{B})\subseteq \mathcal{B}\) where \(\mathcal{B}\) is the
Borel \(\sigma\)-algebra on \(\R\). Here we do not have ``\(\mathcal{F}\)'' as
the default \(\sigma\)-algebra on \(\R\) is always chosen to be
\(\mathcal{B}\).
...
\begin{enumerate}[label={(\arabic*)}]
\item \(\mu_X\) is a function from \(\mathcal{B}\) to \([0,1]\). \cmark
...
\item \textbf{Properties of random variables.}
...
=X^{-1}(\{x_{i_1}\}\cup\dotsb X^{-1}(\{x_{i_j}\})
\in\mathcal{F}
\)
...
\begin{pf}
Fix any \(B\in\mathcal{B}\). Then, \(\vc{f^{-1}(B)}\in\mathcal{B}\) as \(f\) is
measurable. Since \(X\) is a \(\mathcal{F}\)-random variable,
\(X^{-1}(\vc{f^{-1}(B)})\in\mathcal{F}\). The result then follows from the fact that
...
\end{center}
\end{itemize}
\item \textbf{Definition of Lebesgue integral.}
...
\end{pf}
\item \textbf{Definition and properties of expectation.} With the knowledge of
...
\begin{enumerate}
\item \textbf{Preliminary terms.} In \Cref{subsect:conv-thm}, we are going to
...
\item \emph{(for functions on \(\R\))}
Let \(\{f_n\}\) be a sequence of measurable real-valued functions on \(\R\) converging almost everywhere to
another measurable real-valued function \(f\) on \(\R\). If we have \(|f_n|\le g\) almost
everywhere for every \(n\in\N\), where \(g\) is a function such that \(
\int_{-\infty}^{\infty}g(x)\odif{x}<\infty\), then
\[\lim_{n\to\infty}\int_{-\infty}^{\infty}f_n(x)\odif{x}
=\int_{-\infty}^{\infty}\lim_{n\to\infty}f_n(x)\odif{x}
=\int_{-\infty}^{\infty}f(x)\odif{x}.\]
\item \emph{(for random variables)}
...
probability space as others) with \(\expv{Y}<\infty\), then
...
\(\int_{\R}^{}g(x)\odif{\mu_X(x)}=\int_{-\infty}^{\infty}g(x)f(x)\odif{x}\)
(details omitted).
\end{enumerate}
\subsection{Change of Measure}
\begin{enumerate}
\item We have now reached the last subsection in \Cref{sect:prob-theory}, which
is also perhaps the most important one here, due to its foundational role for
...
so-called \emph{risk-neutral measure}).
...
\(Z(\omega)\prob{\{\omega\}}=\tpr{\{\omega\}}\), for all \(\omega\in\Omega\).

However, difficult arises when \(\Omega\) is uncountable and
\(\prob{\{\omega\}}=\tprob{\{\omega\}}=0\) for all \(\omega\in\Omega\). In such
case, \emph{any} random variable \(Z\) would satisfy \(Z(\omega)\prob{\{\omega\}}=\tpr{\{\omega\}}\)
...
\begin{pf}
\begin{enumerate}[label={(\arabic*)}]
\item Since we have \(\indic_{A}Z\ge 0\) almost surely, by
\labelcref{it:lebesgue-int-prop} we have \(\tprob{A}\ge 0\). Also, we have
\(\indic_{A}Z\le Z\), thus \(\tprob{A}=\int_{\Omega}^{}\indic_{A}Z(\omega)\odif{\prob{\omega}}
\le\int_{\Omega}^{}Z(\omega)\odif{\prob{\omega}}
=\expv{Z}=1
\) by \labelcref{it:lebesgue-int-prop}. \cmark
\item We have
...
measurable functions \faIcon{arrow-right} (4) general measurable functions.
...
simple functions \(\{X_n\}\) such that \(0\le X_1\le X_2\le\dotsb\le X\) and
...
\item Similarly for (b), we can apply it for any general random variable \(Y\)
...
assign zero probability to the same collection of events in \(\mathcal{F}\).
\begin{note}
...
and the Radon-Nikodym theorem, we may write \(\displaystyle
Z(\omega)=:\frac{\odif{\tprob{\omega}}}{\odif{\prob{\omega}}}\) for all
...
\(\tpr\) with respect to \(\pr\)}, which is often denoted by \(\displaystyle
\frac{\odif{\tpr}}{\odif{\pr}}\).
\end{enumerate}
